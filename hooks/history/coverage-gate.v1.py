...
def changed_files(root):
    rc1, tracked = sh("git diff --name-only HEAD", root)
...
def executed(output):
    ...
    got |= set(re.findall(r"^(@?[\w.-]+(?:/[\w.-]+)?)\s+test\$", output, re.M))  # pnpm -r
    return got
...
def main():
    ...
    if not cons:
        ...
        save({"tree": th, "result": "pass", "why": "변경에 의존하는 다른 워크스페이스 없음"})
        sys.exit(0)
    ...
    vkey = hashlib.sha256(json.dumps(sorted(gaps), ensure_ascii=False).encode()).hexdigest()[:12]
    ...
    save({"tree": th, "result": "block", "gaps": sorted(gaps), "ran": sorted(ran), "count": count})

    lines = ["[coverage-gate] 완료 전에 확인할 것 — 테스트 명령이 실제로 돌리지 않은 곳에 네 변경의 소비자가 있다.", ""]
